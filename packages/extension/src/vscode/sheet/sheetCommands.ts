...
export function registerSheetCommands(
  ...
  const disposables: vscode.Disposable[] = [
    ...
    vscode.commands.registerCommand('sqlEngineLab.runSql', async () => {
      ...
      if (!profile) {
        profile = await configureConnectionForSheet(
          editor.document.uri,
          currentEngineId,
          vault,
          sheetManager,
    ...
    vscode.commands.registerCommand('sqlEngineLab.openTutorialSheet', async (moduleId: string) => {
      ...
      if (!tutorialsData) {
        try {
          ...
          void vscode.window.showErrorMessage(
            `No se pudo cargar lab-tutorials.json: ${err.message}`,
          );
          return;
        }
      }
      ...
      if (!engineTutorials || !engineTutorials[moduleId]) {
        void vscode.window.showErrorMessage(
          `Tutorial '${moduleId}' no encontrado para el motor ${currentEngineId}`,
        );
        return;
      }
 ...
 */
function getSqlResultWebviewContent(): string {
  return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
